(function installRadarProntuarioConditionalReconciler(root, factory) {
    ...
    const getServiceAdvisoryState = serviceAdvisory?.getServiceAdvisoryState
        || ((invoice = {}, fallback = {}) => ({
            sent: typeof invoice.consultaAssessoriaEnviada === 'boolean'
                ? invoice.consultaAssessoriaEnviada
                : fallback.sent === true,
            analysis: invoice.analiseConsultaAssessoria || fallback.analysis || 'Não analisado'
        }));
    ...
    function syncProgramVisualState(root, schoolId, compKey, state, verification, rows) {
        const { competence: competenceKey, programId } = splitContext(root, compKey);
    ...
    function reconcile(root, schoolId, compKey) {
        ...
        syncProgramVisualState(root, schoolId, compKey, state, verification, rows);
    ...
    function forceProntuarioRender(root, schoolId, args = []) {
        if (!originalRenderProntuario) return false;
        const result = originalRenderProntuario(schoolId, ...args);
    ...
    function installRenderDispatcher(root) {
        ...
        root.renderProntuario = function renderProntuarioWithConditionalReconciliation(schoolId, ...args) {
            if ((suppressedSchools.get(text(schoolId)) || 0) > 0) return false;
            return forceProntuarioRender(root, schoolId, args);
        };
        Object.defineProperty(root.renderProntuario, '__radarConditionalReconciler', {
            value: true,
            enumerable: false
        });
        return true;
