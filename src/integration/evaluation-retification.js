(function installRadarEvaluationRetification(root, factory) {
    ...
    const TECHNICAL_VALUES = new Set(['Não analisado', 'Correto', 'Correto (Atrasado)']);
    ...
    function assertExpectedVersion(verification, input) {
        if (input.expectedVerificationVersion != null
            && input.expectedVerificationVersion !== rowVersionOf(verification)) {
            fail('OPTIMISTIC_CONFLICT', 'A avaliação mudou desde que você abriu a edição. Feche e abra novamente para conferir o valor atual.', 'retifyEvaluation');
        }
    }

    function assertRetifiableBonification(documentKey) {
        if (!Object.hasOwn(DOCUMENT_LABELS, documentKey) || DERIVED_BONIFICATION.has(documentKey)) {
            fail(
    ...
    function assertRetifiableTechnicalDocument(documentKey) {
        if (!Object.hasOwn(DOCUMENT_LABELS, documentKey)) {
            fail('DOCUMENT_NOT_APPLICABLE', 'Documento não disponível para edição.', 'correctTechnicalAnalysis');
        }
        ...
        return service.runSerializedVerificationWrite(input, async () => {
            ...
            return service.dataService.execute({
                ...
                mutate: () => {
                    ...
                    assertExpectedVersion(verification, input);
                    if (service.findActivePendency(state, schoolId, compKey, documentKey)) {
                        fail('ACTIVE_PENDENCY', 'Existe Pendência ativa neste documento. Use Editar análise para corrigir o lançamento e justificar sua anulação antes de desfazer a entrega.', 'undoBonification');
                    }
                    if (documentKey === 'notaFiscal' && list(state.registeredInvoices).some(note => (
                        text(note.escolaId || note.school_id) === schoolId && text(note.compKey) === compKey
                    ))) {
                        fail('FISCAL_NOTES_EXIST', 'Existem despesas cadastradas. Edite cada nota individualmente antes de desfazer a marcação de entrega.', 'undoBonification');
                    }
    ...
    async function correctTechnicalAnalysis(service, input = {}) {
        ...
        if (!TECHNICAL_VALUES.has(requestedValue)) {
            fail('INVALID_ANALYSIS', 'Selecione uma análise técnica válida.', 'correctTechnicalAnalysis');
        }
