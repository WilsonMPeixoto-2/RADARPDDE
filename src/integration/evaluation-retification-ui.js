(function installRadarEvaluationRetificationUi(root, factory) {
    ...
    const DERIVED_BONIFICATION_DOCUMENTS = new Set(['boletoInternet', 'consAssessoria', 'consEnviada']);
    ...
    function closeModal(root) {
        ...
        if (!layer || layer.querySelector('form[aria-busy="true"]')) return false;
        layer.remove();
    ...
    function buildModal(root, mode, row, context, trigger) {
        ...
        const activePendency = activePendencyFor(root, context);
        const expectedVerificationVersion = verification.rowVersion ?? verification.row_version ?? null;
        ...
        layer.innerHTML = `
            ...
            <section class="evaluation-retification-dialog" role="dialog" aria-modal="true" aria-labelledby="evaluation-retification-title" aria-describedby="evaluation-retification-guidance">
        ...
        let saving = false;
        let committedNeedsRefresh = false;
        const preview = layer.querySelector('.evaluation-retification-preview');

        const refresh = () => {
            if (saving || committedNeedsRefresh) return;
            ...
            const undoBlocked = !isAnalysis && !nextValue && Boolean(activePendency);
            submit.disabled = !changed || !completeFormalConfirmation || undoBlocked;
            preview.textContent = changed
                ? `${currentDisplay} → ${nextValue || 'Não preenchido'}${!isAnalysis && !nextValue ? ' · Análise: Não analisado' : ''}`
                : 'Escolha um novo valor para habilitar a gravação.';
            feedback.textContent = undoBlocked
                ? 'Existe Pendência ativa. Primeiro use Editar análise para corrigir o lançamento e justificar a anulação da Pendência.'
                : '';
        };
        ...
        layer.addEventListener('keydown', event => {
            if (event.key === 'Tab') {
                const focusable = Array.from(layer.querySelectorAll('button:not(:disabled), select:not(:disabled), input:not(:disabled), textarea:not(:disabled)'))
        ...
        form.addEventListener('submit', async event => {
            event.preventDefault();
            if (submit.disabled || saving || committedNeedsRefresh) return;
            const nextValue = text(valueSelect.value);
            const requiresFormalConfirmation = Boolean(
                isAnalysis
                && activePendency
            ...
            saving = true;
            layer.querySelectorAll('button, input, select, textarea').forEach(control => { control.disabled = true; });
            submit.textContent = 'Salvando edição…';
            ...
            try {
                ...
                if (result.stateSync?.refreshRequired || result.refreshPending || result.stateApplyErrorCode) {
                    committedNeedsRefresh = true;
                    feedback.textContent = 'A alteração foi salva no Supabase, mas a tela não conseguiu atualizar os dados. Atualize a página para conferir o resultado. Não repita a gravação.';
                ...
                closeModal(root);
                // Render only the current school/context from the confirmed in-memory projection.
                // Never repaint a different month if navigation occurred during the write.
                if (currentSchoolId(root) === context.schoolId && currentCompetence(root) === context.competence) {
                    root.rebuildOperationalIndexes?.();
                    if (typeof root.renderProntuario === 'function') root.renderProntuario(context.schoolId);
                    else syncRowFromState(root, row, context);
                }
                ...
                saving = false;
                layer.querySelectorAll('button, input, select, textarea').forEach(control => { control.disabled = false; });
                submit.removeAttribute('aria-busy');
                form.removeAttribute('aria-busy');
                refresh();
    ...
    function decorateProntuarioRows(root) {
        ...
        rows.forEach(row => {
            ...
            if (!context) return;
            const verification = verificationFor(root, context) || {};
            const future = root.RadarCompetencia?.isFutureCompetence?.(context.competence) === true;
            if (future) return;
            ...
            const consolidated = Boolean(verification.resultadoBonif || verification.bonus_result) && currentProfile(root) !== 'assistente';
            if (consolidated) row.querySelector('[data-evaluation-retification-actions="bonification"]')?.remove();
            if (bonificationGroup && !consolidated && !DERIVED_BONIFICATION_DOCUMENTS.has(context.documentKey)) {
