...
export function validateMatrix(matrix, rootDir = DEFAULT_ROOT) {
    ...
    for (const operation of matrix.operations || []) {
        ...
        if (!VALID_CRITICALITIES.has(operation.criticality)) {
            findings.push(`${prefix}: criticidade desconhecida ${operation.criticality}.`);
        }
        if (!coverageStates.has(operation.coverage)) {
            findings.push(`${prefix}: cobertura desconhecida ${operation.coverage}.`);
        }
        if (!Object.hasOwn(evidenceSets, operation.evidence)) {
            findings.push(`${prefix}: conjunto de evidências desconhecido ${operation.evidence}.`);
        }
        ...
        if (!Array.isArray(operation.resources) || operation.resources.length === 0) {
            findings.push(`${prefix}: resources deve conter ao menos um recurso.`);
        }
        if (!Array.isArray(operation.gaps)) findings.push(`${prefix}: gaps deve ser um array.`);
        if (operation.coverage === 'covered' && operation.gaps?.length > 0) {
            findings.push(`${prefix}: operação coberta não pode declarar lacunas.`);
        }
        if (operation.coverage !== 'covered' && operation.gaps?.length === 0) {
            findings.push(`${prefix}: cobertura ${operation.coverage} exige lacuna explícita.`);
        }
...
function renderProfiles(matrix) {
    const rows = matrix.profiles
        .map(profile => `| \`${profile.id}\` | ${escapeCell(profile.label)} | ${escapeCell(profile.kind)} |`)
        .join('\n');
    return [
        '## Perfis',
        '',
...
function renderOperations(matrix) {
    ...
    for (const surface of matrix.surfaces) {
        ...
        sections.push('', `### ${surfaceLabels.get(surface.id)}`, '',
...
function renderGaps(matrix) {
    ...
    for (const operation of matrix.operations) {
        ...
        for (const gap of operation.gaps) {
            rows.push(`- **${operation.id} — ${operation.action}:** ${gap}`);
        }
    }
    return [
