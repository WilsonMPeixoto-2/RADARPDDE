...
test('contrato integral migra, retoma, reconcilia e reverte contra Supabase local', async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    const entities = {
      ...
      pendencies: [{ id: 'FULL-P', school_id: 'FULL-SCHOOL', competence_origin: '2032-01', program_id: 'FULL', document_key: 'ata', registered_invoice_id: null, status: 'Resolvida', responsible_area: '', next_actor: '', reason: '', notes: 'Regularizada', opened_at: fixed, resolved_at: fixed, canceled_at: null, payload: {}, row_version: 1, created_at: fixed, updated_at: fixed }],
      pendencyAttempts: [{ id: 'FULL-A', pendency_id: 'FULL-P', attempt_number: 1, available_at: null, submitted_at: fixed, analyzed_at: fixed, result: 'correto', observation: '', drive_url: '', errors: [], payload: {}, created_by: null, row_version: 1, created_at: fixed, updated_at: fixed }],
