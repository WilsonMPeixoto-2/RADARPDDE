...
begin
    ...
    if (v_result = 'correto' and (nullif(p_pendency ->> 'status', '') <> 'Resolvida' or nullif(p_pendency ->> 'resolved_at', '') is null or nullif(p_pendency ->> 'canceled_at', '') is not null))
       or (v_result in ('incorreto', 'arquivo_indisponivel') and (nullif(p_pendency ->> 'status', '') <> 'Aberta' or nullif(p_pendency ->> 'resolved_at', '') is not null or nullif(p_pendency ->> 'canceled_at', '') is not null)) then
    ...
    update public.pendencies set status = p_pendency ->> 'status', responsible_area = coalesce(p_pendency ->> 'responsible_area', responsible_area),
        next_actor = coalesce(p_pendency ->> 'next_actor', next_actor), reason = coalesce(p_pendency ->> 'reason', reason), notes = coalesce(p_pendency ->> 'notes', notes),
        resolved_at = nullif(p_pendency ->> 'resolved_at', '')::timestamptz, canceled_at = null, payload = coalesce(p_pendency -> 'payload', payload)
    ...
    update public.pendency_attempts set analyzed_at = (p_attempt ->> 'analyzed_at')::timestamptz, result = v_result,
        errors = coalesce(p_attempt -> 'errors', errors), payload = v_attempt_payload where id = v_existing_attempt.id returning * into v_attempt;
    update public.verifications set analysis = coalesce(p_verification_patch -> 'analysis', analysis), bonification = coalesce(p_verification_patch -> 'bonification', bonification),
    ...
    values (p_administrative_log ->> 'id', v_existing_pendency.school_id, auth.uid(), coalesce(p_administrative_log ->> 'user_identifier', ''),
        coalesce(p_administrative_log ->> 'profile_name', public.current_app_role(), ''), p_administrative_log ->> 'action',
        coalesce(p_administrative_log -> 'details', '{}'::jsonb), coalesce(nullif(p_administrative_log ->> 'event_at', '')::timestamptz, now()));
