...
-- Também fecha o ciclo atômico NF -> resumo mensal -> pendência -> histórico.
...
-- A RPC genérica passa a preservar a identidade da NF quando a pendência é de
-- Assessoria. Os demais documentos mantêm exatamente o contrato anterior.
...
begin
    ...
    if v_registered_invoice_id is not null then
        if p_pendency ->> 'document_key' <> 'consAssessoria' then
            raise exception 'VALIDATION_ERROR: somente pendência de Assessoria pode referenciar NF';
        end if;
        ...
        if v_invoice.expense_type <> 'servico'
            or v_invoice.school_id is distinct from v_school_id
...
-- Confirmação do modal: NF individual, resumo mensal, pendência e histórico são
-- uma única transação. Se qualquer etapa falhar, nenhuma delas permanece.
...
begin
    if v_invoice_id is null
        ...
        raise exception 'VALIDATION_ERROR: pendência de Assessoria deve apontar para a NF atualizada';
    end if;
    if nullif(p_invoice ->> 'expense_type', '') <> 'servico'
        or p_invoice #>> '{payload,analiseConsultaAssessoria}' <> 'Incorreto' then
        raise exception 'VALIDATION_ERROR: abertura atômica exige NF de serviço com Assessoria Incorreta';
    end if;
...
-- Na reanálise, apenas a NF vinculada é alterada. O serviço fornece o novo
-- estado individual e a projeção agregada; ambas persistem junto com a
-- transição da pendência, tentativa e histórico na mesma transação.
...
begin
    if v_invoice_id is null
        or v_pendency_invoice_id is distinct from v_invoice_id
        or nullif(p_pendency ->> 'document_key', '') <> 'consAssessoria' then
        raise exception 'VALIDATION_ERROR: reanálise de Assessoria deve apontar para a NF vinculada';
    end if;
    if nullif(p_invoice ->> 'expense_type', '') <> 'servico'
        or v_analysis not in ('Correto', 'Correto (Atrasado)', 'Incorreto') then
        raise exception 'VALIDATION_ERROR: estado individual de reanálise da Assessoria inválido';
    end if;
