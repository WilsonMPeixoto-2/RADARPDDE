...
-- Demanda do help desk: registrar observação sem concluir a inventariação.
select lives_ok($sql$
    select public.save_asset_with_log(
...
$sql$, 'Inventário da mesma CRE salva somente a observação pela RPC real');
...
$sql$, 'Inventário da mesma CRE conclui o bem depois da observação');
