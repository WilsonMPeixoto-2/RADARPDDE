-- RADAR PDDE — limpeza auditável do lançamento de teste NF 1234.
-- Solicitação explícita do usuário em 22/09/2026.
-- Escopo fechado: E.M. Ema Negrão de Lima (04.10.001), 2026-09, BASIC.
-- Idempotente para reexecução após sucesso e fail-closed diante de qualquer divergência.
  ...
  if not found then
    if exists (
      ...
      where id = 'pend-4c35085f-3b5a-4077-bc23-bf5ceecfafde'
         or registered_invoice_id = 'nota-8f25ad3a-da84-4867-a1cf-e2acbfbd2fdd'
    ) then
      raise exception 'IDEMPOTENCY_CONFLICT: NF ausente, mas ainda existe Pendência vinculada.';
    end if;
    ...
    if not found
       or v_after.bonification ->> 'consAssessoria' <> 'Não se aplica'
       or coalesce((v_after.bonification ->> 'consEnviada')::boolean, true) <> false
       or v_after.analysis ->> 'consAssessoria' <> 'Correto'
       or v_after.analysis ->> 'notaFiscal' <> 'Não analisado'
       or not exists (
         select 1
         from public.administrative_logs
         where id = 'log-cleanup-test-invoice-1234-20260922'
       ) then
      raise exception 'IDEMPOTENCY_CONFLICT: NF ausente sem estado pós-limpeza comprovado.';
    end if;

    raise notice 'NOOP: lançamento de teste já foi removido e o estado final está íntegro.';
  else
    if v_invoice.school_id <> '04.10.001'
    ...
    if (select count(*) from public.registered_invoices
        where school_id = '04.10.001'
          and competence_id = '2026-09'
          and program_id = 'BASIC') <> 1 then
    ...
    where id = v_pendency.id
      and row_version = v_pendency.row_version;
    ...
    raise notice 'CLEANUP_OK: NF %, Pendência % e tentativa vinculada removidas; verificação % recomposta.',
      v_invoice.id, v_pendency.id, v_verification.id;
  end if;
end
$$;
...
select
  (select count(*) from public.registered_invoices
   where id = 'nota-8f25ad3a-da84-4867-a1cf-e2acbfbd2fdd') as invoice_remaining,
  (select count(*) from public.pendencies
   where id = 'pend-4c35085f-3b5a-4077-bc23-bf5ceecfafde'
      or registered_invoice_id = 'nota-8f25ad3a-da84-4867-a1cf-e2acbfbd2fdd') as pendency_remaining,
  (select count(*) from public.pendency_attempts
   where pendency_id = 'pend-4c35085f-3b5a-4077-bc23-bf5ceecfafde') as attempts_remaining,
  (select count(*) from public.administrative_logs
   where id = 'log-cleanup-test-invoice-1234-20260922') as cleanup_log_count,
  v.row_version as verification_row_version,
  v.bonification ->> 'consAssessoria' as assessoria_delivery,
  v.bonification ->> 'consEnviada' as assessoria_sent,
  v.analysis ->> 'consAssessoria' as assessoria_analysis,
  v.analysis ->> 'notaFiscal' as invoice_analysis
